...
SETUP_TOOLS() {
    cp -fa "target/a53x/patches/dtbo/tools/dtc" \
           "target/a53x/patches/dtbo/tools/mkdtimg" \
           "$TOOLS_DIR"
}
...
APPLY_PATCH() {
    ...
    PATCH="$SRC_DIR/target/a53x/patches/dtbo/patches/$2"
...
REPACK() {
    mkdtimg cfg_create "$TMP_DIR/dtbo.img" "$TMP_DIR/a53x.cfg" -d "$TMP_DIR" &> /dev/null
}
